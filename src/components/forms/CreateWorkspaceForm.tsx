...
export default function CreateWorkspaceForm({
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Successfully created "${settings.name}" workspace!`);
      onSuccess();
    } catch (err: unknown) {
      console.error('Workspace creation error:', err);
      const errorMsg = err instanceof Error ? err.message : 'Something went wrong';
      setError(errorMsg);
      toast.error(`Error creating workspace: ${errorMsg}`);
    } finally {
      setIsSubmitting(false);
    }
  };

  const isFormValid = !settings.name.trim() || !profile.username.trim();
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="flex justify-end space-x-3 pt-4 border-t">
        ...
        >
          {isSubmitting || loading ? 'Creating...' : 'Create Workspace'}
        </Button>
      </div>
    </form>
  );
