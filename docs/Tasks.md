...
| 11.4 | Build Slack Command Handlers | - Implement `/send-karma` endpoint with Slack signature verification<br>- Resolve Slack users to workspace members before calling internal APIs<br>- Parse Slack mention tokens (e.g., `<@U123>`) or raw Slack user IDs to locate the corresponding workspace user before triggering transactions<br>- Respond with guidance when recipients lack Slack identity links | 11.3 | 🔴 To-Do | Developer | 3-4 hours | Medium | Relies on workspace mapping and identity data |
...
| 11.7 | Prepare Supabase Slack Schema Migration | - Review existing database `.sql` migration files for context<br>- Design new schema additions for Slack identities, workspace mapping, and audit data<br>- Author a new migration `.sql` file that preserves current functionality and introduces required fields | 11.2 | 🔴 To-Do | Developer | 3-4 hours | High | Ensure existing flows continue to operate during Slack rollout |
| 11.8 | Handle Slack Event Subscriptions | - Configure event subscription endpoints for user join/leave and app uninstall<br>- Process workspace membership updates and cleanup tokens on uninstall<br>- Maintain audit logs for significant Slack events | 11.6 | 🔴 To-Do | Developer | 3-4 hours | Medium | Extends slash commands with background event handling |
