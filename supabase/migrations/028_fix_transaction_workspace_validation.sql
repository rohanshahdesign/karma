-- 028_fix_transaction_workspace_validation.sql
-- Fix validate_and_create_transaction to ensure correct workspace mapping
-- Ensures transactions are always created with sender's workspace_id and validates receiver belongs to same workspace
...
BEGIN
  ...
  -- Validate receiver exists and belongs to the same workspace (safety check)
  SELECT workspace_id
    INTO v_receiver_workspace_id
  FROM public.profiles
  WHERE id = p_receiver_profile_id;

  IF v_receiver_workspace_id IS NULL THEN
    RAISE EXCEPTION 'Receiver profile not found';
  END IF;

  IF v_receiver_workspace_id != v_workspace_id THEN
    RAISE EXCEPTION 'Receiver profile does not belong to the same workspace';
  END IF;

  -- Prevent self-transactions
  IF v_sender_profile_id = p_receiver_profile_id THEN
    RAISE EXCEPTION 'Cannot send karma to yourself';
  END IF;
  ...
  -- IMPORTANT: Always use sender's workspace_id to ensure correct workspace mapping
  INSERT INTO public.transactions (
    ...
    v_workspace_id,  -- Always use sender's workspace_id
    v_sender_profile_id,
  ...
  -- Update balances (only update profiles in the same workspace)
  UPDATE public.profiles
  SET giving_balance = giving_balance - p_amount
  WHERE id = v_sender_profile_id
    AND workspace_id = v_workspace_id;  -- Extra safety check
